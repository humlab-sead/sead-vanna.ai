...
def train_on_sql():
    ...
    vn.train(
        ...
        sql="""
        SELECT tbl_sites.site_id, tbl_sites.site_name FROM tbl_sites ORDER BY site_name
        """)
    ...
    vn.train(
        ...
        sql="""
        SELECT tbl_sites.site_id, tbl_sites.site_name FROM tbl_sites
    ...
    vn.train(
        ...
        sql="""
        SELECT DISTINCT(tbl_sites.site_id), tbl_sites.site_name FROM
        public.tbl_sites
        JOIN tbl_sample_groups ON tbl_sample_groups.site_id=tbl_sites.site_id
        JOIN tbl_physical_samples ON tbl_physical_samples.sample_group_id=tbl_sample_groups.sample_group_id
        JOIN tbl_analysis_entities ON tbl_analysis_entities.physical_sample_id=tbl_physical_samples.physical_sample_id
    ...
    vn.train(
        ...
        sql="""
        SELECT tbl_measured_values.measured_value, tbl_sites.site_id, tbl_sites.site_name FROM
        public.tbl_sites
        JOIN tbl_sample_groups ON tbl_sample_groups.site_id=tbl_sites.site_id
        JOIN tbl_physical_samples ON tbl_physical_samples.sample_group_id=tbl_sample_groups.sample_group_id
        JOIN tbl_analysis_entities ON tbl_analysis_entities.physical_sample_id=tbl_physical_samples.physical_sample_id
...
# This will break up the information schema into bite-sized chunks that can be referenced by the LLM
#plan = vn.get_training_plan_generic(df_information_schema)
#print(type(df_information_schema))